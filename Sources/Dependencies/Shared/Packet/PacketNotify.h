...
// === NUEVO: PAQUETE DE LOGROS ===
	...
	struct HB_PACKED EnchantBagEntry {
	...
	struct HB_PACKED PacketNotifyEnchantBag {
		PacketHeader header;
		uint16_t count;
		EnchantBagEntry entries[50];
	};

	struct HB_PACKED RecoverQueueEntry {
		uint32_t db_id;
		int16_t item_id;
		uint8_t item_color;
		uint8_t prefix_type;
		...
		uint32_t spec_value1;
		uint32_t spec_value2;
		uint32_t spec_value3;
		uint32_t shards_yield;
		uint32_t fragments_yield;
	};

	struct HB_PACKED PacketNotifyRecoverQueue {
		PacketHeader header;
		uint16_t count;
		RecoverQueueEntry entries[20];
	};
	// =============================
