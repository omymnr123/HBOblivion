...
bool GameCmdGM::execute(CGame* game, int client_h, const char* args)
{
    ...
    if (hb_stricmp(args, "on") == 0)
    {
        ...
        game->m_client_list[client_h]->m_status.level = game->m_client_list[client_h]->m_level;
        game->m_client_list[client_h]->m_status.prestige_level = game->m_client_list[client_h]->m_prestige_level;
