...
bool ItemManager::equip_item_handler(int client_h, short item_index, bool notify)
{
	...
	// Set item color and armor visibility for this equip slot
	{
		...
		// Force +15 Dark weapons to have item_color 9 for the glare effect
		if (item->m_instance.enchant_bonus == 15) {
			if (strncmp(item->m_name, "Dark Knight", 11) == 0 || strncmp(item->m_name, "Dark Mage", 9) == 0) {
				item->m_instance.item_color = 9;
			}
		}
...
void ItemManager::give_item_handler(int client_h, short item_index, int amount, short dX, short dY, uint32_t object_id_full, const char* item_name)
{
	uint16_t object_id = object_id_full & 0xFFFF;
	int page = (object_id_full >> 16) & 0xFFFF;
	...
	if ((m_game->m_client_list[client_h]->m_item_list[item_index]->is_stackable()) &&
		...
		if (owner_h == 0) {
			...
			if (owner_type == hb::shared::owner_class::Player) {
				...
				if (m_game->m_npc_list[owner_h]->m_npc_config_id == 58) { // Warehouse Keeper
					// NPC     .
					if (set_item_to_bank_item(client_h, item, page) == false) {
		...
		if (owner_h == 0) {
			...
			if (owner_type == hb::shared::owner_class::Player) {
				...
				if (m_game->m_npc_list[owner_h]->m_npc_config_id == 58) { // Warehouse Keeper
					if (set_item_to_bank_item(client_h, item_index, page) == false) { // <--- AÑADIR , page 
						m_game->send_notify_msg(0, client_h, Notify::CannotItemToBank, 0, 0, 0, 0);
...
void ItemManager::request_retrieve_item_handler(int client_h, char* data)
{
	...
	if (!pkt) return;
	bank_item_index = static_cast<int>(pkt->item_slot);
	//wh remove
...
bool ItemManager::set_item_to_bank_item(int client_h, short item_index, int page)
{
	int ret;
	CItem* item;
	...
	if (page < 0 || page > 9) page = 0;

	int start_slot = page * 30;
	int end_slot = start_slot + 30;

	for (int i = start_slot; i < end_slot && i < hb::shared::limits::MaxBankItems; i++) {
		if (m_game->m_client_list[client_h]->m_item_in_bank_list[i] == 0) {
...

			{
				...
				pkt.bank_index = static_cast<uint16_t>(i);
				pkt.is_new = 1;
...
// Apply skill multiplier with difficulty scaling
    // As skill level increases from 0 to 100, the multiplier decreases from 1.0x to 0.0x
    ...
    // Seleccionar el multiplicador correcto según la habilidad
    float active_multiplier = m_game->m_weapon_skill_multiplier; // Por defecto usamos el de armas/general
...
bool ItemManager::set_item_to_bank_item(int client_h, CItem* item, int page)
{
	int ret;
...
void ItemManager::req_sell_item_handler(int client_h, char item_id, char sell_to_whom, int num, const char* item_name)
{
    ...
    case 24:
        if (target_item->m_durability > 0) {
            ...
            if (remain_life == 0) {
                m_game->send_notify_msg(0, client_h, Notify::CannotSellItem, item_id, 2, 0, target_item->m_name);
                break;
            }

            d1 = (double)remain_life;
            if (target_item->m_durability != 0)
                d2 = (double)target_item->m_durability;
            else d2 = 1.0f;
            ...
            if (m_game->m_client_list[client_h]->m_cur_weight_load + get_item_weight(m_pGold, price) > m_game->calc_max_load(client_h)) {
                m_game->send_notify_msg(0, client_h, Notify::CannotSellItem, item_id, 4, 0, target_item->m_name);
            }
            else m_game->send_notify_msg(0, client_h, Notify::SellItemPrice, item_id, remain_life, price, target_item->m_name, num);
        }
        else {
            ...
            else m_game->send_notify_msg(0, client_h, Notify::SellItemPrice, item_id, 0, price, target_item->m_name, num);
        }
        break;
		...
		if ((m_game->m_client_list[client_h]->m_item_list[item_index] != 0) &&
			...
			case ItemEffectType::Attack:
				...
				case ItemEffectType::AttackManaSave:
					...
					if (strncmp(m_game->m_client_list[client_h]->m_item_list[item_index]->m_name, "Dark Mage", 9) == 0 ||
						strncmp(m_game->m_client_list[client_h]->m_item_list[item_index]->m_name, "Dark Knight", 11) == 0)
					{
						m_game->m_client_list[client_h]->m_mana_save_ratio += m_game->m_client_list[client_h]->m_item_list[item_index]->m_instance.enchant_bonus;
					}
...
case ItemEffectType::Dye:
        if ((dest_item_id >= 0) && (dest_item_id < hb::shared::limits::MaxItems)) {
            ...
            // Permite tinte normal en capas, ropa, zapatos, etc., ignorando si la BD olvidó poner is_dyeable = 1.
            // Bloquea estrictamente solo las armaduras pesadas (armor) y las armas.
            if (dest && (is_dye_removal || (dest->m_armor_class != armor_class::armor && !is_weapon_slot(dest->get_equip_pos())))) {
...
{
    // --- NUEVO: El Oro (ID 10) ya no pesa absolutamente nada ---
    if (item != nullptr && item->m_id_num == hb::shared::item::ItemId::Gold) return 0;
...
void ItemManager::request_item_upgrade_handler(int client_h, int item_index)
{
	...
	int upgrade_route;
	if (upgrade_ist == hb::shared::item::item_sub_type::accessory || upgrade_ist == hb::shared::item::item_sub_type::angelic)
	...
	case 46: // Pendants/accessories
		...
		if (!hb::shared::item::is_accessory_slot(m_game->m_client_list[client_h]->m_item_list[item_index]->get_equip_pos()))
		{
			m_game->send_notify_msg(0, client_h, Notify::ItemUpgradeFail, 2, 0, 0, 0);
			return; // Pendants are accessory slots
		}
		...
		case 19: // AngelicPendant(MAG)
			...
			// 100% success rate for Angelic Pendants
	...
	case 1: // weapons upgrade
		...
		case 745: // DarkKnightHammer
			...
			if ((value == 0) && m_game->m_client_list[client_h]->m_item_list[item_index]->m_id_num == 703)
			{
				...
				m_game->m_client_list[client_h]->m_item_list[item_index]->m_instance.enchant_bonus = value;
				if (value == 15) {
					m_game->m_client_list[client_h]->m_item_list[item_index]->m_instance.item_color = 9;
				}
			...
			else if ((value == 7) && (m_game->m_client_list[client_h]->m_item_list[item_index]->m_id_num == 727))
			{
				item_x = m_game->m_client_list[client_h]->m_item_pos_list[item_index].x;
				item_y = m_game->m_client_list[client_h]->m_item_pos_list[item_index].y;
	...
	case 8: // wands upgrade 
		...
		case 746:
			...
			if ((value == 5) && (m_game->m_client_list[client_h]->m_item_list[item_index]->m_id_num == 714))
			{
				item_x = m_game->m_client_list[client_h]->m_item_pos_list[item_index].x;
				item_y = m_game->m_client_list[client_h]->m_item_pos_list[item_index].y;
			...
			else if ((value == 9) && (m_game->m_client_list[client_h]->m_item_list[item_index]->m_id_num == 738))
			{
				item_x = m_game->m_client_list[client_h]->m_item_pos_list[item_index].x;
				item_y = m_game->m_client_list[client_h]->m_item_pos_list[item_index].y;
...
bool ItemManager::reroll_item_attributes(int client_h, int item_index, uint64_t gold_cost)
{
	if (m_game == nullptr || m_game->m_client_list[client_h] == nullptr) return false;
	...
	// Check if item is an equipment that supports rare attributes
	if (item->get_item_effect_type() != ItemEffectType::Attack &&
		item->get_item_effect_type() != ItemEffectType::Defense &&
		item->get_item_effect_type() != ItemEffectType::AttackManaSave)
	{
		m_game->send_notify_msg(0, client_h, Notify::NoticeMsg, 0, 0, 0, "Este item no admite reroll de stats.");
		return false;
	}
	...
	// Deduct gold
	m_game->calc_total_weight(set_item_count_by_id(client_h, hb::shared::item::ItemId::Gold, current_gold - gold_cost));

	// Reset item base stats before applying new attributes
	if (item->m_id_num >= 0 && item->m_id_num < hb::server::config::MaxItemTypes && m_game->m_item_config_list[item->m_id_num] != nullptr)
	{
		CItem* config = m_game->m_item_config_list[item->m_id_num];
		item->m_weight = config->m_weight;
		item->m_durability = config->m_durability;
		item->m_swing_speed = config->m_swing_speed;
	}

	item->m_instance.prefix_type = 0;
	item->m_instance.prefix_value = 0;
	item->m_instance.secondary_type = 0;
	item->m_instance.secondary_value = 0;
	item->m_instance.item_color = 0;

	// Roll new primary and secondary attributes
	generate_item_attributes(item);

	// Ensure secondary effect is guaranteed on Blacksmith rerolls
	if (item->m_instance.secondary_type == static_cast<uint8_t>(SecondaryEffectType::None)) {
		if (item->get_item_effect_type() == ItemEffectType::Defense) {
			int secRoll = rand() % 10001;
			int secCumul = 0;
			struct { int weight; SecondaryEffectType type; int minVal; int maxVal; } defenseSecondary[] = {
				{ 1000, SecondaryEffectType::DefenseRatio,      3, 13 },
			...
			for (auto& entry : defenseSecondary) {
				secCumul += entry.weight;
				if (secRoll < secCumul) {
					item->m_instance.secondary_type = static_cast<uint8_t>(entry.type);
					item->m_instance.secondary_value = static_cast<uint8_t>(roll_attribute_value(entry.minVal, entry.maxVal));
					break;
				}
			}
		} else if (item->get_item_effect_type() == ItemEffectType::Attack || item->get_item_effect_type() == ItemEffectType::AttackManaSave) {
			int secRoll = rand() % 10000;
			int secCumul = 0;
			struct { int weight; SecondaryEffectType type; int minVal; int maxVal; int fixedVal; } attackSecondary[] = {
				{ 4999, SecondaryEffectType::HittingProb,       3, 13, 0 },
			...
			for (auto& entry : attackSecondary) {
				...
				if (secRoll < secCumul) {
					item->m_instance.secondary_type = static_cast<uint8_t>(entry.type);
					if (entry.fixedVal > 0) item->m_instance.secondary_value = entry.fixedVal;
					else item->m_instance.secondary_value = static_cast<uint8_t>(roll_attribute_value(entry.minVal, entry.maxVal));
					break;
				}
			}
		}
	...
	// Recalculate equipped items if item is equipped
	if (item->m_equip_pos != 0) {
		validate_equipped_items(client_h);
	}

	// Update client item attributes
	...
	std::string success_msg = std::format("¡Reroll exitoso en {}! Se descontaron {} de oro.", item->m_name, gold_cost);
	m_game->send_notify_msg(0, client_h, Notify::NoticeMsg, 0, 0, 0, success_msg.c_str());
