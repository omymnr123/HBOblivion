...
bool EnsureAccountDatabase(const char* account_name, sqlite3** outDb, std::string& outPath)
{
    ...
    const char* schemaSql =
        ...
        " FOREIGN KEY(character_name) REFERENCES characters(character_name) ON DELETE CASCADE"
        ");"
        "CREATE TABLE IF NOT EXISTS character_enchant_bag ("
        " character_name TEXT NOT NULL,"
        " material_type INTEGER NOT NULL,"
        " stat_id INTEGER NOT NULL,"
        " level INTEGER NOT NULL,"
        " amount INTEGER NOT NULL,"
        " PRIMARY KEY(character_name, material_type, stat_id, level),"
        " FOREIGN KEY(character_name) REFERENCES characters(character_name) ON DELETE CASCADE"
        ");"
        "CREATE TABLE IF NOT EXISTS character_recover_queue ("
        " id INTEGER PRIMARY KEY AUTOINCREMENT,"
        " character_name TEXT NOT NULL,"
        " item_id INTEGER NOT NULL,"
        " item_color INTEGER NOT NULL DEFAULT 0,"
        ...
        " secondary_value INTEGER NOT NULL DEFAULT 0,"
        " enchant_bonus INTEGER NOT NULL DEFAULT 0,"
        " shards_yield INTEGER NOT NULL DEFAULT 0,"
        " fragments_yield INTEGER NOT NULL DEFAULT 0,"
        " FOREIGN KEY(character_name) REFERENCES characters(character_name) ON DELETE CASCADE"
        ");"
...
bool ListCharacterSummaries(sqlite3* db, const char* account_name, std::vector<AccountDbCharacterSummary>& outChars)
{
    ...
    const char* sql =
        "SELECT character_name, underwear_type, hair_color, hair_style, skin_color, gender, skin, level, exp, prestige_level, map_name "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        row.prestige_level = static_cast<int32_t>(sqlite3_column_int(stmt, 9));
        CopyColumnText(stmt, 10, row.map_name, sizeof(row.map_name));
...
bool InsertCharacterState(sqlite3* db, const AccountDbCharacterState& state)
{
    ...
    // --- ¡AQUÍ ESTABA EL BUG QUE FALTABA! ---
    ok &= (sqlite3_bind_int(stmt, col++, state.prestige_level) == SQLITE_OK);
    ok &= (sqlite3_bind_int(stmt, col++, state.prestige_bonus_stats) == SQLITE_OK);
...
{
    outBag.clear();
    const char* sql = "SELECT material_type, stat_id, level, amount FROM character_enchant_bag WHERE character_name = ?;";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        hb::logger::error("Prepare error in LoadCharacterEnchantBag: {}", sqlite3_errmsg(db));
        return false;
    }
    sqlite3_bind_text(stmt, 1, character_name, -1, SQLITE_STATIC);

    while (sqlite3_step(stmt) == SQLITE_ROW) {
        AccountDbEnchantBagRow row;
        row.material_type = sqlite3_column_int(stmt, 0);
        row.stat_id = sqlite3_column_int(stmt, 1);
        row.level = sqlite3_column_int(stmt, 2);
        row.amount = sqlite3_column_int(stmt, 3);
        outBag.push_back(row);
    }
    sqlite3_finalize(stmt);
    return true;
}
...
{
    const char* delSql = "DELETE FROM character_enchant_bag WHERE character_name = ?;";
    sqlite3_stmt* delStmt;
    if (sqlite3_prepare_v2(db, delSql, -1, &delStmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(delStmt, 1, character_name, -1, SQLITE_STATIC);
    ...
    const char* insSql = "INSERT INTO character_enchant_bag (character_name, material_type, stat_id, level, amount) VALUES (?, ?, ?, ?, ?);";
    sqlite3_stmt* insStmt;
    if (sqlite3_prepare_v2(db, insSql, -1, &insStmt, nullptr) != SQLITE_OK) {
        hb::logger::error("Prepare error in InsertCharacterEnchantBag: {}", sqlite3_errmsg(db));
        return false;
    }

    for (const auto& row : bag) {
        sqlite3_bind_text(insStmt, 1, character_name, -1, SQLITE_STATIC);
        sqlite3_bind_int(insStmt, 2, row.material_type);
        sqlite3_bind_int(insStmt, 3, row.stat_id);
        sqlite3_bind_int(insStmt, 4, row.level);
        sqlite3_bind_int(insStmt, 5, row.amount);
        if (sqlite3_step(insStmt) != SQLITE_DONE) {
            hb::logger::error("Step error in InsertCharacterEnchantBag: {}", sqlite3_errmsg(db));
        }
...
{
    outQueue.clear();
    const char* sql = "SELECT id, item_id, item_color, spec_effect_value1, spec_effect_value2, spec_effect_value3, prefix_type, prefix_value, secondary_type, secondary_value, enchant_bonus, shards_yield, fragments_yield FROM character_recover_queue WHERE character_name = ? ORDER BY id ASC;";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        hb::logger::error("Prepare error in LoadCharacterRecoverQueue: {}", sqlite3_errmsg(db));
        return false;
    }
    sqlite3_bind_text(stmt, 1, character_name, -1, SQLITE_STATIC);

    while (sqlite3_step(stmt) == SQLITE_ROW) {
        AccountDbRecoverQueueRow row;
        row.db_id = sqlite3_column_int(stmt, 0);
        row.item_id = sqlite3_column_int(stmt, 1);
        row.item_color = sqlite3_column_int(stmt, 2);
        row.spec_effect_value1 = sqlite3_column_int(stmt, 3);
        row.spec_effect_value2 = sqlite3_column_int(stmt, 4);
        row.spec_effect_value3 = sqlite3_column_int(stmt, 5);
        row.prefix_type = sqlite3_column_int(stmt, 6);
        row.prefix_value = sqlite3_column_int(stmt, 7);
        row.secondary_type = sqlite3_column_int(stmt, 8);
        row.secondary_value = sqlite3_column_int(stmt, 9);
        row.enchant_bonus = sqlite3_column_int(stmt, 10);
        row.shards_yield = sqlite3_column_int(stmt, 11);
        row.fragments_yield = sqlite3_column_int(stmt, 12);
        outQueue.push_back(row);
    }
    sqlite3_finalize(stmt);
    return true;
}
...
{
    const char* delSql = "DELETE FROM character_recover_queue WHERE character_name = ?;";
    sqlite3_stmt* delStmt;
    if (sqlite3_prepare_v2(db, delSql, -1, &delStmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_text(delStmt, 1, character_name, -1, SQLITE_STATIC);
        sqlite3_step(delStmt);
    ...
    const char* insSql = "INSERT INTO character_recover_queue (character_name, item_id, item_color, spec_effect_value1, spec_effect_value2, spec_effect_value3, prefix_type, prefix_value, secondary_type, secondary_value, enchant_bonus, shards_yield, fragments_yield) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    sqlite3_stmt* insStmt;
    if (sqlite3_prepare_v2(db, insSql, -1, &insStmt, nullptr) != SQLITE_OK) {
        hb::logger::error("Prepare error in InsertCharacterRecoverQueue: {}", sqlite3_errmsg(db));
        return false;
    }

    for (const auto& row : queue) {
        sqlite3_bind_text(insStmt, 1, character_name, -1, SQLITE_STATIC);
        sqlite3_bind_int(insStmt, 2, row.item_id);
        sqlite3_bind_int(insStmt, 3, row.item_color);
        sqlite3_bind_int(insStmt, 4, row.spec_effect_value1);
        sqlite3_bind_int(insStmt, 5, row.spec_effect_value2);
        sqlite3_bind_int(insStmt, 6, row.spec_effect_value3);
        sqlite3_bind_int(insStmt, 7, row.prefix_type);
        sqlite3_bind_int(insStmt, 8, row.prefix_value);
        sqlite3_bind_int(insStmt, 9, row.secondary_type);
        sqlite3_bind_int(insStmt, 10, row.secondary_value);
        sqlite3_bind_int(insStmt, 11, row.enchant_bonus);
        sqlite3_bind_int(insStmt, 12, row.shards_yield);
        sqlite3_bind_int(insStmt, 13, row.fragments_yield);
        if (sqlite3_step(insStmt) != SQLITE_DONE) {
            hb::logger::error("Step error in InsertCharacterRecoverQueue: {}", sqlite3_errmsg(db));
        }
        sqlite3_reset(insStmt);
    }
    sqlite3_finalize(insStmt);
...
{
    const char* sql = "DELETE FROM character_recover_queue WHERE id = ?;";
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        hb::logger::error("Prepare error in DeleteCharacterRecoverQueue: {}", sqlite3_errmsg(db));
        return false;
    }

    sqlite3_bind_int(stmt, 1, db_id);
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        hb::logger::error("Step error in DeleteCharacterRecoverQueue: {}", sqlite3_errmsg(db));
        sqlite3_finalize(stmt);
        return false;
    }
