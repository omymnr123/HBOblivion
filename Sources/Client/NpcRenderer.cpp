...
hb::shared::sprite::BoundRect CNpcRenderer::draw_damage(int indexX, int indexY, int sX, int sY, bool trans, uint32_t time)
{
	...
	if (frame < 4)
	{
		...
		else if (state.m_owner_type == hb::shared::owner::ForgottenSealedTomb ||
				 state.m_owner_type == hb::shared::owner::AncientSealedTomb ||
				 state.m_owner_type == hb::shared::owner::PrimordialSealedTomb) npcPose = 4;
		else npcPose = 0;
	}
	else
	{
...
hb::shared::sprite::BoundRect CNpcRenderer::draw_damage_move(int indexX, int indexY, int sX, int sY, bool trans, uint32_t time)
{
	...
	if (state.m_owner_type == hb::shared::owner::Wyvern) npcPose = 0;
			 ...
			 state.m_owner_type == hb::shared::owner::AncientSealedTomb ||
			 state.m_owner_type == hb::shared::owner::PrimordialSealedTomb) npcPose = 0;
	else npcPose = 3;
...
hb::shared::sprite::BoundRect CNpcRenderer::draw_dying(int indexX, int indexY, int sX, int sY, bool trans, uint32_t time)
{
	...
	if (!trans)
	{
		...
		{
			if (state.m_status.hero) {
				RenderHelpers::draw_body(m_game, eq.m_body_index + (state.m_dir - 1), sX, sY, frame, false, state.m_owner_type, state.m_status.frozen, false, true);
			} else {
				RenderHelpers::draw_body(m_game, eq.m_body_index + (state.m_dir - 1), sX, sY, frame, false, state.m_owner_type, state.m_status.frozen, false, false);
			}
		}
...
hb::shared::sprite::BoundRect CNpcRenderer::draw_dead(int indexX, int indexY, int sX, int sY, bool trans, uint32_t time)
{
	...
	if (!trans)
	{
		if (state.m_frame == -1)
		{
			...
			state.m_frame = frame;
			RenderHelpers::draw_body(m_game, eq.m_body_index + (state.m_dir - 1), sX, sY, frame, false, state.m_owner_type, state.m_status.frozen, false, state.m_status.hero);
		}
