...
void DialogBox_SysMenu::draw_graphics_tab(short sX, short sY)
{
	...
	if (is_item_visible(lineY))
	{
		put_string(labelX, lineY, "Hover Health Bar:", GameColors::UILabel);
		put_string(labelX + 1, lineY, "Hover Health Bar:", GameColors::UILabel);
		draw_toggle(smallBoxX, lineY, config_manager::get().is_hover_health_bar_enabled());
	}
...
bool DialogBox_SysMenu::on_click_general(short sX, short sY)
{
		...
		mouse_y >= buttonY && mouse_y <= buttonY + ui_layout::btn_size_y)
	{
		...
		{
			if (m_game->on_game()->m_logout_count == -1) {
				m_game->on_game()->m_logout_count = m_game->is_fast_logout_map() ? 4 : 11;
...
bool DialogBox_SysMenu::on_click_graphics(short sX, short sY)
{
	...
	// Hover Health Bar toggle
	if (is_item_visible(lineY))
	{
		if (is_in_toggle_area(smallBoxX, lineY)) {
			bool enabled = config_manager::get().is_hover_health_bar_enabled();
			config_manager::get().set_hover_health_bar_enabled(!enabled);
			audio_manager::get().play_game_sound(sound_type::effect, 14, 5);
			return true;
		}
	}
