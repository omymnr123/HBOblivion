...
void DialogBox_GuideMap::draw_zoomed_map(short sX, short sY)
{
	...
	if (m_game->m_sarcofago_id >= 150 && m_game->m_sarcofago_id <= 152)
	{
		if ((m_game->m_cur_time % 500) < 370)
		{
			if (m_game->m_sarcofago_x >= shX && m_game->m_sarcofago_x <= shX + 128 &&
				m_game->m_sarcofago_y >= shY && m_game->m_sarcofago_y <= shY + 128)
			{
				m_game->m_sprite[InterfaceNdCrusade]->draw(sX + m_game->m_sarcofago_x - shX, sY + m_game->m_sarcofago_y - shY, 40);
			}
		}
	}
}
...
void DialogBox_GuideMap::draw_full_map(short sX, short sY)
{
	...
	if (m_game->m_sarcofago_id >= 150 && m_game->m_sarcofago_id <= 152)
	{
		if ((m_game->m_cur_time % 500) < 370)
		{
			shX = (m_game->m_sarcofago_x * 128) / (m_game->m_map_data->m_map_size_x);
			shY = (m_game->m_sarcofago_y * 128) / (m_game->m_map_data->m_map_size_y);
			m_game->m_sprite[InterfaceNdCrusade]->draw(sX + shX, sY + shY, 40);
		}
	}
}
