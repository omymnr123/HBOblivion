	...
	case 0:
		for (i = 0; i < 17; i++)
			if (((i + m_scroll_position) < hb::shared::limits::MaxSkillType) && (m_game->m_skill_cfg_list[i + m_scroll_position] != 0))
			{
				...
				bool clickable = ((m_game->m_skill_cfg_list[i + m_scroll_position]->m_is_useable == true)
					&& (m_game->m_skill_cfg_list[i + m_scroll_position]->m_level != 0)) || (i + m_scroll_position == 24);
			...
			if (((i + m_scroll_position) < hb::shared::limits::MaxSkillType) && (m_game->m_skill_cfg_list[i + m_scroll_position] != 0))
			{
				if ((mouse_x >= sX + 25) && (mouse_x <= sX + 166) && (mouse_y >= sY + 45 + i * 15) && (mouse_y <= sY + 59 + i * 15))
				{
					...
					if (clickable)
					{
						...
						if ((i + m_scroll_position) == 24) // Enchanting Skill ID
						{
							audio_manager::get().play_game_sound(sound_type::effect, 12, 5);
							m_game->get_dialog_box_manager().toggle_dialog_box(DialogBoxId::Enchanting);
