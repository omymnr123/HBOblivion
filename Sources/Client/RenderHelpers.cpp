                    ...
                    bool inv, int colorIndex)
{
	...
	if (!sprites.contains(spriteIndex)) {
		hb::logger::log("Sprite index out of bounds: {}", spriteIndex);
		return;
	}
                ...
                int weaponFrame, bool inv)
{
	...
	if (!sprites.contains(eq.m_weapon_index)) {
		hb::logger::log("Sprite index out of bounds: {}", eq.m_weapon_index);
		return;
	}
                ...
                int frame, bool inv)
{
	...
	if (!sprites.contains(eq.m_shield_index)) {
		hb::logger::log("Sprite index out of bounds: {}", eq.m_shield_index);
		return;
	}
...
void draw_body(CGame& game, int body_dir_index, int sX, int sY, int frame,
              bool inv, short owner_type, bool frozen, bool admin_invis, bool has_aura)
{
	int draw_x = sX;
	int draw_y = sY;

	if (owner_type == hb::shared::owner::ForgottenSealedTomb ||
		owner_type == hb::shared::owner::AncientSealedTomb ||
		owner_type == hb::shared::owner::PrimordialSealedTomb)
	{
		// Offset the Sarcofago sprite so it doesn't look like it's floating
		draw_y += 28; 
		draw_x -= 10;
	}
	...
	{
		game.m_sprite[body_dir_index]->draw(draw_x, draw_y, frame, hb::shared::sprite::DrawParams::alpha_blend(0.5f));
	}
	else if (admin_invis)
	{
		game.m_sprite[body_dir_index]->draw(draw_x, draw_y, frame,
	...
	if (owner_type == hb::shared::owner::ManaStone ||
		owner_type == hb::shared::owner::ForgottenSealedTomb ||
		owner_type == hb::shared::owner::AncientSealedTomb ||
		owner_type == hb::shared::owner::PrimordialSealedTomb ||
		has_aura)
	{
		// Draw a subtle aura underneath the ManaStone using an existing effect sprite
		// Effect sprite 26 or 27 is usually an aura
		game.m_effect_sprites[26]->draw(sX, sY, (int)((timeGetTime() % 3000) / 100), hb::shared::sprite::DrawParams::alpha_blend(0.6f));
	}
                      ...
                      bool admin_invis)
{
	...
	if (weapon_draw_order[dir] == 1)
	{
		...
		draw_body(game, bodyDirIndex, sX, sY, frame, inv, state.m_owner_type, state.m_status.frozen, admin_invis, state.m_status.hero);
                   ...
                   bool inv)
{
	...
	draw_body(game, bodyDirIndex, sX, sY, frame, inv, state.m_owner_type, state.m_status.frozen, false, state.m_status.hero);
}
