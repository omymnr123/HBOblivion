...
void Screen_OnGame::register_hotkeys()
{
	...
	hotkeys.register_hotkey({ KeyCode::L, ctrlOnly.ctrl, ctrlOnly.shift, ctrlOnly.alt }, HotkeyManager::Trigger::KeyUp,
		[this]()
		{
	...
	hotkeys.register_hotkey({ KeyCode::G, ctrlOnly.ctrl, ctrlOnly.shift, ctrlOnly.alt }, HotkeyManager::Trigger::KeyUp,
		[this]()
		{
			if (GameModeManager::get_mode() != GameMode::MainGame || !hb::shared::input::is_ctrl_down())
				return;
			if (get_dialog_box_manager().is_enabled(DialogBoxId::Guild) == true) get_dialog_box_manager().disable_dialog_box(DialogBoxId::Guild);
			else get_dialog_box_manager().enable_dialog_box(DialogBoxId::Guild, 0, 0, 0, 0);
		});
	hotkeys.register_hotkey({ KeyCode::E, ctrlOnly.ctrl, ctrlOnly.shift, ctrlOnly.alt }, HotkeyManager::Trigger::KeyUp,
		[this]()
		{
			if (!hb::shared::input::is_ctrl_down() || GameModeManager::get_mode() != GameMode::MainGame || text_input_manager::get().is_active())
				return;
			get_dialog_box_manager().toggle_dialog_box(DialogBoxId::Enchanting);
			get_dialog_box_manager().toggle_dialog_box(DialogBoxId::EnchantingBag);
		});
...
#else
				int logout_val = m_game->is_fast_logout_map() ? 4 : 11;
				if (m_logout_count == -1 || m_logout_count > logout_val)
				{
					m_logout_count = logout_val;
					m_logout_count_time = GameClock::get_time_ms();
				}
#endif
			}
