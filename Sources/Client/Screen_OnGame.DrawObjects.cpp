		...
		for (int tileX = startTileX; tileX <= endTileX; tileX++)
		{
			...
			if ((ix >= visMinX) && (ix <= visMaxX) && (iy >= visMinY) && (iy <= visMaxY))
			{
				...
				else
				{
					...
					if (m_game->m_entity_state.m_action < 0 || m_game->m_entity_state.m_action > 10) {
						m_game->m_entity_state.m_action = 0; // Type::stop
					}
					...
					if (m_game->m_entity_state.m_dir < 1 || m_game->m_entity_state.m_dir > 8) {
						m_game->m_entity_state.m_dir = direction::south;
					}
...
void Screen_OnGame::draw_object_name(short screen_x, short screen_y, const char* name, const hb::shared::entity::PlayerStatus& status, uint16_t object_id)
{
	...
	bool show_health_bar = config_manager::get().is_hover_health_bar_enabled() && status.hp_percent > 0 && status.hp_percent <= 100;
	if (object_id == m_game->m_player->m_player_object_id && m_party_status == 0) {
	...
	int next_text_y = screen_y + 26 + y_offset;

	if (status.guild_rank > 0 && status.guild_name[0] != '\0')
	{
		std::string guild_rank_str;
		switch (status.guild_rank)
		{
	...
	if (status.gm_mode)
	{
		hb::shared::text::draw_text(GameFont::Default, screen_x, next_text_y, "Game Master", hb::shared::text::TextStyle::with_shadow(hb::shared::render::Color(255, 150, 0)));
		next_text_y += 12;
	}
	else
	{
		std::string levelPrestigeText;
		if (status.prestige_level > 0) {
			levelPrestigeText = std::format("Lvl.{} (+{})", status.level, status.prestige_level);
		} else {
			levelPrestigeText = std::format("Lvl.{}", status.level);
		}

		hb::shared::text::draw_text(GameFont::Default, screen_x, next_text_y, levelPrestigeText.c_str(), hb::shared::text::TextStyle::with_shadow(hb::shared::render::Color(255, 255, 0)));
		next_text_y += 12;
	}
}
...
void Screen_OnGame::dk_glare(int weapon_color, int16_t weapon_item_id, int* weapon_glare)
{
	...
	if (strncmp(cfg->m_name, "Dark Knight", 11) == 0)
	{
		*weapon_glare = 3;
	}
	else if (strncmp(cfg->m_name, "Dark Mage", 9) == 0)
	{
		*weapon_glare = 2;
	}
}
