	...
	: IDialogBox(DialogBoxId::Bank, game)
{
	...
	m_item_count = 13; // Número de líneas visibles en pantalla por página
}
...
void DialogBox_Bank::on_draw()
{
	...
	// Dibujar los números romanos de las 10 páginas en la parte inferior
	draw_roman_numerals(sX, sY, size_x);
...
void DialogBox_Bank::draw_roman_numerals(short sX, short sY, short size_x)
{
	const char* roman[10] = { "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X" };
	int start_x = sX + 18;
	int y_pos = sY + 300; // Posición inferior fija en la ventana

	for (int p = 0; p < 10; p++) {
		int roman_x = start_x + (p * 22);
		if (m_current_page == p) {
			put_string(roman_x, y_pos, roman[p], GameColors::UIItemName_Special);
		} else {
			put_string(roman_x, y_pos, roman[p], GameColors::UIBlack);
		}
	}
}
...
void DialogBox_Bank::draw_item_list(short sX, short sY, short size_x)
{
	...
	int page_start = m_current_page * m_items_per_page; // Cada página muestra 30 elementos (Ej: P0 = 0-29, P1 = 30-59)

	for (int i = 0; i < m_item_count; i++) {
		int itemIndex = page_start + i + m_scroll_offset;
		if ((itemIndex < page_start + m_items_per_page) && (itemIndex < hb::shared::limits::MaxBankItems) && (player().m_bank_list[itemIndex] != 0)) {
	...
	// Contar elementos totales dentro de esta página para la barra lateral
	int total_lines = 0;
	for (int i = 0; i < m_items_per_page; i++) {
		int idx = page_start + i;
		if (idx < hb::shared::limits::MaxBankItems && player().m_bank_list[idx] != 0) total_lines++;
	}
...
bool DialogBox_Bank::on_click()
{
	...
	// Detección de clics en los números romanos inferiores (cambio de página)
	int start_x = sX + 18;
	int y_pos = sY + 300;
	if (mouse_y >= y_pos - 2 && mouse_y <= y_pos + 14) {
		for (int p = 0; p < 10; p++) {
			int roman_x = start_x + (p * 22);
			if (mouse_x >= roman_x - 2 && mouse_x <= roman_x + 18) {
				m_current_page = p;
				m_scroll_offset = 0; // Resetear scroll interno al cambiar de página
	...
	case mode::list:
		for (int i = 0; i < m_item_count; i++) {
			if ((mouse_x > sX + 30) && (mouse_x < sX + 210) && (mouse_y >= sY + 110 + i * 15) && (mouse_y <= sY + 124 + i * 15)) {
				int page_start = m_current_page * m_items_per_page;
				int itemIndex = page_start + m_scroll_offset + i;
				if ((itemIndex < page_start + m_items_per_page) && (itemIndex < hb::shared::limits::MaxBankItems) && (player().m_bank_list[itemIndex] != 0)) {
					...
					}
					{
						...
						req.item_slot = static_cast<uint16_t>(itemIndex);
...
bool DialogBox_Bank::on_item_drop()
{
	...
	if (player().m_item_list[give.item_index] == nullptr) return false;
	if (inventory_manager::get().is_locked(give.item_index)) return false;
	...
	// CAMUFLAMOS LA PÁGINA EN EL PACKET PARA QUE EL SERVIDOR LO LEA
	uint32_t packed_target_id = give.object_id | (m_current_page << 16);

	if ((cfg->is_stackable()) && (player().m_item_list[give.item_index]->m_instance.count > 1))
	{
		...
		dropDlg->m_drop_x = give.target_x;
		dropDlg->m_drop_y = give.target_y;
		dropDlg->m_drop_target_type = 1002;
		dropDlg->m_drop_target_id = packed_target_id; // <-- ID + Página
	...
	else
	{
		if (inventory_manager::get().get_bank_item_count() >= (m_game->m_max_bank_items - 1)) {
			...
			pkt.v4 = packed_target_id; // <-- ID + Página
