...
int inventory_manager::calc_total_weight()
{
    ...
    {
        // Usamos .get() para extraer la dirección de memoria del unique_ptr de forma segura
        CItem* item = m_game->m_player->m_item_list[i].get();
        if (!item) continue;
        
        // --- NUEVO: El Cliente ignora el Oro al calcular el peso de la mochila ---
        if (item->m_id_num == hb::shared::item::ItemId::Gold) continue;

        // Usamos .get() también aquí por el mismo motivo
        CItem* cfg = m_game->m_item_config_list[item->m_id_num].get();
        if (!cfg) continue;
