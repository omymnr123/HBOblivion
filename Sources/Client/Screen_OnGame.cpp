                        ...
                        if ((absX == 0) && (absY == 0))
                            ...
                            case 1002:
                                if (inventory_manager::get().get_bank_item_count() >= (m_game->m_max_bank_items - 1)) m_game->add_event_list(DLGBOX_CLICK_NPCACTION_QUERY9, 10);
                                    ...
                                    auto* dropDlg = m_game->get_dialog_box_manager().get_dialog_as<DialogBox_ItemDropAmount>(DialogBoxId::ItemDropExternal);
                                    CItem* cfg = m_game->get_item_config(m_game->m_player->m_item_list[dropDlg->m_item_index]->m_id_num);
                                    if (cfg)
                                    {
                                    	auto pkt = hb::net::make_common_command_str(CommonType::GiveItemToChar, m_game->m_player->m_player_x, m_game->m_player->m_player_y, static_cast<char>(dropDlg->m_item_index));
                                    	pkt.v1 = amount;
                                    	pkt.v2 = dropDlg->m_drop_x;
                                    	pkt.v3 = dropDlg->m_drop_y;
                                    	std::snprintf(pkt.text, sizeof(pkt.text), "%s", cfg->m_name);
                                    	pkt.v4 = dropDlg->m_drop_target_id;
                                    	m_game->send_game_packet(pkt);
                                    }
                                }
                                break;
...
void Screen_OnGame::render_item_tooltip()
{
    ...
    if (cfg->get_equip_pos() == EquipPos::RightHand || cfg->get_equip_pos() == EquipPos::TwoHand)
    {
        ...
        if (item->m_instance.enchant_bonus > 0 && 
            (strncmp(cfg->m_name, "Dark Knight", 11) == 0 || strncmp(cfg->m_name, "Dark Mage", 9) == 0))
        {
            range.min += item->m_instance.enchant_bonus;
            range.max += item->m_instance.enchant_bonus;
        }
...
void Screen_OnGame::draw_tile_grid()
{
    ...
    int colorIndex = config_manager::get().get_tile_grid_color();
    bool isTrans = config_manager::get().is_tile_grid_trans();
    
    int iGridR, iGridG, iGridB;
    switch (colorIndex) {
    case 1:
        iGridR = isTrans ? 60 : 110; iGridG = 18; iGridB = 18;
        break;
    case 2:
        iGridR = 18; iGridG = isTrans ? 60 : 110; iGridB = 18;
        break;
    case 3:
        iGridR = 18; iGridG = 18; iGridB = isTrans ? 70 : 120;
        break;
    case 4:
        iGridR = isTrans ? 70 : 135; iGridG = isTrans ? 70 : 135; iGridB = isTrans ? 70 : 135;
        break;
    default:
        iGridR = isTrans ? 24 : 48; iGridG = isTrans ? 24 : 48; iGridB = isTrans ? 24 : 48;
        break;
    }

    // Use alpha 255 if not transparent, or ~150 if transparent to simulate the old game's look
    uint8_t alpha = isTrans ? 150 : 255;
    hb::shared::render::Color gridColor(iGridR, iGridG, iGridB, alpha);

    int screenW = LOGICAL_WIDTH();
    int screenH = LOGICAL_HEIGHT();

    for (int iy = -m_sModY; iy < (screenH - 6); iy += 32)
    {
        for (int ix = -m_sModX; ix < (screenW + 47); ix += 32)
        {
            m_game->m_Renderer->draw_line(ix - 16, iy - 16, ix + 16, iy - 16, gridColor);
            m_game->m_Renderer->draw_line(ix - 16, iy - 16, ix - 16, iy + 16, gridColor);
        }
    }
}
