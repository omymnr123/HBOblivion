...
void config_manager::set_defaults()
{
	...
	m_tile_grid_color = 0;   // Default color
	m_tile_grid_trans = true; // Default transparent mode
...
bool config_manager::load(const char* filename)
{
	...
	try
	{
		...
		if (j.contains("display"))
		{
			...
			if (display.contains("tileGridColor"))
			{
				m_tile_grid_color = display["tileGridColor"].get<int>();
			}
			if (display.contains("tileGridTrans"))
			{
				m_tile_grid_trans = display["tileGridTrans"].get<bool>();
			}
...
bool config_manager::save(const char* filename)
{
	...
	j["display"]["tileGridColor"] = m_tile_grid_color;
	j["display"]["tileGridTrans"] = m_tile_grid_trans;
