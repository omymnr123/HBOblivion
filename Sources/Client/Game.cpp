...
void CGame::on_event(const hb::shared::render::event& e)
{
    ...
    case hb::shared::render::event_id::closed:
        ...
        {
            // Si estamos dentro del juego, interceptamos el cierre para que 
            // actúe igual que el menú de sistema (F12) en lugar de cerrar de golpe.
        ...
        {
            // Fuera del juego (en menús de login, selección, etc.), se cierra con normalidad.
            request_quit();
        }
        break;
...
void CGame::init_item_list(char* packet_data)
{
	...
	for (int i = 0; i < total_items; i++)
	{
		...
		int slot = static_cast<int>(entry.slot);
		if (slot < 0 || slot >= hb::shared::limits::MaxBankItems) continue;
		m_player->m_bank_list[slot] = std::make_unique<CItem>();
		m_player->m_bank_list[slot]->m_id_num = entry.item_id;
		m_player->m_bank_list[slot]->m_instance.count = entry.count;
		m_player->m_bank_list[slot]->m_x = 40;
		m_player->m_bank_list[slot]->m_y = 30;
		m_player->m_bank_list[slot]->m_instance.cur_durability = entry.cur_durability;
		m_player->m_bank_list[slot]->m_instance.item_color = entry.item_color;
		m_player->m_bank_list[slot]->m_instance.special_effect_value2 = static_cast<short>(entry.spec_value2); // v1.41
		m_player->m_bank_list[slot]->load_attributes_from(entry);
	}
...
void CGame::retrieve_item_handler(char* data)
{
	...
	int bank_item_index, item_index;
	int j;
	const auto* header = hb::net::PacketCast<hb::net::PacketHeader>(
		data, sizeof(hb::net::PacketHeader));
	if (!header) return;
	if (header->msg_type != MsgType::Reject)
	{
		...
		if (!pkt) return;
		bank_item_index = static_cast<int>(pkt->bank_index);
		item_index = static_cast<int>(pkt->item_index);
...
bool CGame::is_fast_logout_map() const
{
	static const char* fast_maps[] = {
		"arewrhus", "bsmith_1", "bsmith_1f", "cath_1", "cityhall_1", 
		"cmdhall_1", "gldhall_1", "gshop_1", "gshop_1f", "resurr1", 
		"wrhus_1", "wrhus_1f", "wzdtwr_1", "bsmith_2", "bsmith_2f", 
		"cath_2", "cityhall_2", "cmdhall_2", "elvwrhus", "gldhall_2", 
		"gshop_2", "gshop_2f", "resurr2", "wrhus_2", "wrhus_2f", "wzdtwr_2"
	};

	for (const char* m : fast_maps)
	{
		if (_stricmp(m_map_name.c_str(), m) == 0)
			return true;
	}
	return false;
}
...
{
	if (!m_init_data_ready || !m_configs_ready) return; // Prevent client processing during load screen
...
int CGame::has_hero_set(const hb::shared::entity::PlayerAppearance& appr, short OwnerType)
{
	...
	int16_t hero_helm = appr.helm_item_id;
	int16_t hero_armor = appr.armor_item_id;
	int16_t hero_hauberk = appr.arm_item_id;
	int16_t hero_leggings = appr.pants_item_id;

	if (hero_helm > 0 && hero_armor > 0 && hero_hauberk > 0 && hero_leggings > 0)
	{
		// Aresden Warrior (M)
		if (hero_helm == 403 && hero_armor == 411 && hero_hauberk == 419 && hero_leggings == 423) return 1;
		// Aresden Mage (M)
		if (hero_helm == 407 && hero_armor == 415 && hero_hauberk == 419 && hero_leggings == 423) return 2;
		// Aresden Warrior (W)
		if (hero_helm == 404 && hero_armor == 412 && hero_hauberk == 420 && hero_leggings == 424) return 1;
		// Aresden Mage (W)
		if (hero_helm == 408 && hero_armor == 416 && hero_hauberk == 420 && hero_leggings == 424) return 2;

		// Elvine Warrior (M)
		if (hero_helm == 405 && hero_armor == 413 && hero_hauberk == 421 && hero_leggings == 425) return 1;
		// Elvine Mage (M)
		if (hero_helm == 409 && hero_armor == 417 && hero_hauberk == 421 && hero_leggings == 425) return 2;
		// Elvine Warrior (W)
		if (hero_helm == 406 && hero_armor == 414 && hero_hauberk == 422 && hero_leggings == 426) return 1;
		// Elvine Mage (W)
		if (hero_helm == 410 && hero_armor == 418 && hero_hauberk == 422 && hero_leggings == 426) return 2;
	}

	int16_t armor_set = get_set(hero_armor);
	if (armor_set == 0) return 0;
	int16_t leg_set = get_set(hero_leggings);
	int16_t hat_set = get_set(hero_helm);
	int16_t berk_set = get_set(hero_hauberk);
